  ...
  if (self.bitstampRate === 0 || currentTime >= (self.timestamp + self.currencyDelay)) {
    ...
    request('https://www.bitstamp.net/api/ticker/', function(err, response, body) {
      ...
      if (!err && response.statusCode === 200) {
        ...
        request('https://api.hitbtc.com/api/2/public/ticker/SMARTBTC', function(err, response, body) {
          if (err) {
            self.node.log.error(err);
          }
          if (!err && response.statusCode === 200) {
            self.bitstampRate = self.bitstampRate * parseFloat(JSON.parse(body).ask);
          }
        });
      }
