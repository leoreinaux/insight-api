...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.smartcashd.height - blockCached.height + 1;
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.smartcashd.getBlockHeader(hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.smartcashd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.smartcashd.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }
      ...
      self.node.services.smartcashd.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.smartcashd.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  var more = false;
  var moreTimestamp = parseInt(req.query.startTimestamp) + 86400 || lte;

  self.node.services.smartcashd.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getBlockReward = function(height) {
  ...
  var subsidy = new BN(Math.floor((0.5+((5000 * 143500)/(height +1)))) * 1e8);  
